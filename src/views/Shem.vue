<template>
  <section>
    <div class="cards">
      <a data-fancybox="gallery" href="img/shems/schet.png" class="card">
        <img src="img/shems/icons/schet.svg" width="100%" alt="Схема подключения электросчетчика">
        <h4 class="mt-8">Элетро&shy;счетчик</h4>
      </a>

      <a data-fancybox="gallery" href="img/shems/prohod.png" class="card">
        <img src="img/shems/icons/prohod.svg" width="100%" alt="Схема подключения проходного выключателя">
        <h4 class="mt-8">Проход&shy;ной<br>выклю&shy;чатель</h4>
      </a>
    </div>
  </section>
</template>

<script>
  export default {
    name: 'Shem'
  }
</script>